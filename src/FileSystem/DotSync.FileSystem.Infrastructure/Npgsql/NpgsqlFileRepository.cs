...
namespace com.brettnamba.DotSync.FileSystem.Infrastructure.Npgsql;
...
public sealed class NpgsqlFileRepository : IFileRepository
{
    ...
    {
        await using var command = new NpgsqlCommand(FilesInsertCommand, _connection, _transaction);
    ...
    {
        await using var command = new NpgsqlCommand(FilesUpdateCommand, _connection, _transaction);
        command.Parameters.Add(new NpgsqlParameter { Value = file.Id, DbType = DbType.Guid });
        command.Parameters.Add(new NpgsqlParameter { Value = file.Path.Value, DbType = DbType.String });
        command.Parameters.Add(new NpgsqlParameter { Value = file.IsVerified, DbType = DbType.Boolean });
        command.Parameters.Add(new NpgsqlParameter { Value = file.LastSync, DbType = DbType.DateTimeOffset });
    ...
    {
        await using var command = new NpgsqlCommand(FilesQueryByChecksum, _connection, _transaction);
    ...
    {
        await using var command = new NpgsqlCommand(FilesQueryByPath, _connection, _transaction);
        command.Parameters.Add(new NpgsqlParameter { Value = path, DbType = DbType.String });
    ...
    public async Task SetAllAsUnverified(FileSystemPath path)
    {
        if (path.Value != "")
        {
            await using var command = new NpgsqlCommand(FilesSetAsUnverifiedByPathCommand, _connection, _transaction);
            command.Parameters.Add(new NpgsqlParameter { Value = path.Value, DbType = DbType.String });
            await command.ExecuteScalarAsync();
            return;
        }

        await using var command2 = new NpgsqlCommand(FilesSetAllAsUnverifiedCommand, _connection, _transaction);
        await command2.ExecuteScalarAsync();
    }

    public Task<IEnumerable<DotFile>> GetUnverifiedFiles()
    {
        throw new NotImplementedException();
    }

    public Task<IEnumerable<DotFile>> GetFilesByPath(FileSystemPath path)
    {
        throw new NotImplementedException();
    }
